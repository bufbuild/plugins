...
const (
	...
	// docs: https://docs.pypi.org/api/simple/
...
func (c *Client) fetchPyPI(ctx context.Context, name string, ignoreVersions map[string]struct{}, maxVersion string) (string, error) {
	...
	// The Simple API JSON response (PEP 691) has a files array. Each file's
	// yanked field is absent/false when available, or a non-empty string reason
	// when yanked (PEP 592).
	var data struct {
		Files []struct {
...
// Per PEP 691, yanked is absent or false when not yanked, or a non-empty
// string (the reason) when yanked.
